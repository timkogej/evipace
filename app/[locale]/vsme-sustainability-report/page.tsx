...
import { vsmeSustainabilityReportContent } from "@/components/evipace/english-commercial/content";
...
export default async function VsmeSustainabilityReportPage({
  params
}: PageProps) {
  const { locale } = await params;
  ...
  const schemaGraph = [
    ...
    buildServiceSchema(
      ...
      "VSME sustainability reporting preparation",
      "Support for SMEs preparing VSME sustainability reporting from company data, existing documents, source evidence, gaps and assumptions."
    ),
  ...
  return (
    <>
      ...
      <EnglishCommercialServicePage content={vsmeSustainabilityReportContent} />
    </>
  );
}
